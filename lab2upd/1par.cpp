...
int main(int argc, char **argv) {
  ...
  if (size > 8) {
    fprintf(stderr, "i only work with size less than 8:(\n");
    return 1;
  }
  if (ISIZE%size) {
    fprintf(stderr, "i only work when ISIZE=k*size:(\n");
    return 1;
  }
  ...
  if (rank == 0) {
//    printf("0 sts\n");
    for (int i=8 + rank; i < ISIZE; i+=size) {
 //     printf("0: %d\n", i);
